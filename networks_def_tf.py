...
def fire_layer(x, filters_squeeze, filters_1x1, filters_3x3,
    ...
    squeeze = Convolution2D(filters_squeeze, (1, 1),
                            ...
                            init=init,
                            activation=act,
                            kernel_regularizer=l1(weight_cost),
                            bias_regularizer=l1(weight_cost),
...
def conv1x1(N_filt, r, name):
    return Convolution2D(N_filt, (1, 1), padding="same", activation="relu",
                         kernel_regularizer=l2(r), name=name)
...
def squeezenet(N_classes, 
               ...
               fire10_512=False,
               fire10_1024=False,
               fire11_1024=False, input_override=None):
    if input_override is None:
        x = Input(shape=inshape)
    else:
        #print(input_override)
        #x = Convolution2D(shape=input_override.shape)(input_override)
...
def Conv2D(c, h, w, strides=(1, 1),  act_fun="relu", border="same", r=1e-4):
    def f(input_):
        return Convolution2D(c, (h, w), subsample=strides,
                             activation=act_fun, padding=border,
...
def vgg_face(weights_path=None, output_layer = "prob", N_classes = 2622, 
    ...
    out = Dense(N_classes, activation='softmax', name=outname,
                kernel_regularizer=l2(r), bias_regularizer=l2(r))(fc7_drop)

    if output_layer == "prob":
        model = Model(inputs=[img], outputs=[out])

    if output_layer == "vae_123":
	model = Model(inputs=[img],
		      outputs=[conv1_1,conv2_1,conv3_1],name="vgg-face")
    if output_layer == "vae_345":
	model = Model(inputs=[img],
		      outputs=[conv3_1,conv4_1,conv5_1],name="vgg-face")

    elif output_layer == "fc6":
        model = Model(inputs=[img], 
          outputs=[conv1_1,conv1_2,conv2_1,conv2_2,conv3_1,conv3_2,conv3_3,conv4_1,
                   conv4_2,conv4_3,conv5_1,conv5_2,conv5_3fc66],name="vgg-face")
    if weights_path:
...
def VGG_19(weights_path=None,output='prob'):
    model = Sequential()
    model.add(ZeroPadding2D((1,1),input_shape=(3,224,224)))
