...
def id_loss1(y_true,y_pred):
	...
	return K.sum(K.sum(K.sum(K.square(y_true-y_pred),axis=-1),axis=-1))
...
end2end = squ(_x_decoded_mean_squash)
end2end.append(_x_decoded_mean_squash)
print(end2end)
#e2emodel = Model(inputs=[x,labels], outputs=end2end)
e2emodel = Model(inputs=[decoder_input_1,cond],outputs=end2end)

vae.compile(optimizer='adam',loss=vae_loss,metrics=[KL_loss,recon_loss])
losses = {'img_out1': vae_loss, 'vgg-face': id_loss1 }
...
e2emodel.compile(optimizer=adam(lr=0.0005),loss=losses, 
loss_weights=dfc_w + [1])
